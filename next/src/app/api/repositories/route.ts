...
export async function GET() {
  ...
  const raw = await githubFetcher(githubFetchUrl, {}, 600).then((res) =>
    res.json(),
  );
  // await fs.writeFile("data_repos.json", JSON.stringify(raw, null, 2));
  ...
  const dataWLang = await Promise.all(
    data.map(async (repo) => {
      const res: Record<string, number> = await githubFetcher(
        repo.languages_url,
        {},
      ).then((res) => res.json());
      return {
